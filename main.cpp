...
struct Solver {
  ...
  Result solve() {
    ...
    FixColumnSolution noarea_fix_sol = solve_noarea_fixed_column();
    STOP_TIMER(2);
    ...
    STOP_TIMER(3);
    debug("dp_score:%lld\n", dp_sol.score());
    if (noarea_result.score() < noarea_fix_sol.score() && noarea_result.score() < dp_sol.score()) {
      // TODO: インクリメンタルに更新可能にする
  ...
  FixColumnSolution improve(FixColumnSolution& sol, int64_t tl) {
    ...
    for (int i = 0; i < D; ++i) {
      ...
      // debug_vec(pena_sep[i]);
      // debug("%d\n", accumulate(pena_sep[i].begin(), pena_sep[i].end(), 0));
    }
  ...
  pair<int, vi> improve_col_dp(int day, vi nis, int w, const vi& sep_cnt_before, const vi& prev_sep_before, const vi& next_sep_before,
                               const vi& sep_cnt_after, const vi& prev_sep_after, const vi& next_sep_after) {
    static vvi dp(1 << 6, vi(W + 1, INF));
    static vvi prev(1 << 6, vi(W + 1, 0));
    static vector<uint64_t> valid(W + 1);
    fill(valid.begin(), valid.end(), 0ull);
    valid[0] = 1;
    for (int i = 1; i < (1 << nis.size()); ++i) {
      fill(dp[i].begin(), dp[i].end(), INF);
    }
    dp[0][0] = 0;
    if (day != 0) {
    ...
    auto update_dp = [&](int cur_used, int new_used, int cy, int ny, int na) {
      int nv = dp[cur_used][cy];
      int a = (ny - cy) * w;
      if (a < na) nv += (na - a) * 100;
      if (day != 0 && ny != W && sep_cnt_before[ny] != sep_cnt_before[ny - 1]) {
        nv -= w * 2;
      ...
      if (nv < dp[new_used][ny]) {
        dp[new_used][ny] = nv;
        prev[new_used][ny] = (cy << 6) | cur_used;
        valid[ny] |= 1ull << new_used;
      }
    };

    const int ni_mask = (1 << nis.size()) - 1;
    for (int y = 0; y < W; ++y) {
      int v = valid[y];
      while (v) {
        int used = __builtin_ctzll(v);
        v &= v - 1;
        int cands = used ^ ni_mask;
        while (cands) {
          int ni = __builtin_ctz(cands);
          int ai = nis[ni];
          int ns = used | (1 << ni);
          cands &= cands - 1;
          int jy = y + (A[day][ai] + w - 1) / w;
          if (jy < W) {
            update_dp(used, ns, y, jy, A[day][ai]);
            int ny = W;
            if (day != 0) ny = next_sep_before[jy];
            if (day != D - 1) ny = min(ny, next_sep_after[jy]);
            update_dp(used, ns, y, ny, A[day][ai]);
          }
          if (jy != y + 1 && jy - 1 < W) {
            update_dp(used, ns, y, jy - 1, A[day][ai]);
          }
          update_dp(used, ns, y, W, A[day][ai]);
          jy = min(jy, W);
          int py = 0;
          if (day != 0) py = prev_sep_before[jy];
          if (day != D - 1) py = max(py, prev_sep_after[jy]);
          if (py > y) {
            update_dp(used, ns, y, py, A[day][ai]);
          }
        }
      }
    }
    ...
    assert(used == 0);
    reverse(hs.begin(), hs.end());
    return make_pair(dp[ni_mask][W], hs);
  }
  ...
  tuple<vvi, int, int> pack_columns(int day, const vvi& nis, const vi& ws) {
    ...
    for (int i = 0; i < col; ++i) {
      ...
      if (W < sum_h) {
        ...
        if (sum_h < W - nis[i].size()) {
          vector<double> ratio;
          for (int v : nis[i]) {
            ratio.push_back(A[day][v]);
          }
          vi add_hs = distribute_len(ratio, W - sum_h);
          for (int j = 0; j < nis[i].size(); ++j) {
            hs[j] += add_hs[j];
          }
        } else {
          for (int j = 0; j < W - sum_h; ++j) {
            hs[nis[i].size() - 1 - j]++;
          }
        }
      }
      ...
      for (int t = 0; t < 10; ++t) {
        for (int col = t == 0 ? 1 : 2; col <= clamp((int)sqrt(N * 3), 4, 9); ++col) {
          ...
          if (pena_area + pena_wall < best_pena_area + best_pena_wall) {
            ...
            sort(best_rect.begin(), best_rect.end());
          }
        }
      }
  ...
  FixColumnSolution solve_dp(ll tl) {
    debugStr("------ solve_dp ------\n");
    ...
    while (true) {
      for (int col = 2; col <= max_col; ++col) {
        ...
        vi ws = distribute_len(ratio, W);
        debug_vec(ws, "ws");
        FixColumnSolution sol = solve_cols(ws);
        debug("score:%lld col:%d max_ratio:%f amp:%f\n", sol.score(), col, max_ratio, amp);
  ...
  FixColumnSolution solve_cols(const vi& ws) {
    // 小さい面積のエリアから順に使う制約の下でDPする解
    const int col = ws.size();
    // まず初日の部屋を各カラムに分配するDP
    ...
    // for (int i = 0; i < col; ++i) {
    //   debug("%d %.1f\n", nis[i + 1], 1.0 * (acc[nis[i + 1]] - acc[nis[i]]) / ws[i]);
    // }
    vector<int64_t> penas(D);
    vvvi hss(D, vvi(col));
    for (int i = 0; i < col; ++i) {
      if (nis[i] == nis[i + 1]) return SOL_EMPTY;
      hss[0][i] = place_separator(vi(A[0].begin() + nis[i], A[0].begin() + nis[i + 1]));
    }
    penas[0] = initial_day_area_pena(hss[0], ws);
    for (int i = 1; i < D; ++i) {
      auto res = solve_single_day(i, hss[i - 1], ws);
      if (res.second == INF) {
        return SOL_EMPTY;
      }
      hss[i] = res.first;
    ...
    for (int day = 0; day < D; ++day) {
      ...
      for (int c = 0; c < col; ++c) {
        int y = 0;
        vi rect_areas;
        for (int i = 0; i < hss[day][c].size(); ++i) {
          int bottom = y + hss[day][c][i];
          int a = (bottom - y) * ws[c];
          rect_areas.push_back(a);
          if (a < A[day][ai]) {
            wall_cost -= (A[day][ai] - a) * 100;
          }
          y = bottom;
          niss[day][c].push_back(ai);
          ai++;
        }
        sort(rect_areas.begin(), rect_areas.end());
        for (int i = 0; i < rect_areas.size(); ++i) {
          if (rect_areas[i] < A[day][niss[day][c][i]]) {
            area_cost += (A[day][niss[day][c][i]] - rect_areas[i]) * 100;
          }
        }
      }
      assert(ai == N);
    }
    return FixColumnSolution(ws, niss, hss, area_cost, wall_cost);
  }

  int64_t initial_day_area_pena(const vvi& hss, const vi& ws) {
    int64_t pena = 0;
    int ai = 0;
    for (int i = 0; i < hss.size(); ++i) {
      for (int j = 0; j < hss[i].size(); ++j) {
        int area = hss[i][j] * ws[i];
        if (area < A[0][ai]) {
  ...
  pair<vvi, int> solve_single_day(int day, const vvi& prev_hss, const vi& ws) {
    // debug("solve_single_day:%d\n", day);
    const int FAIL = (int)min((int64_t)10000000ll, best_score);
    const int col = prev_hss.size();
    ...
    for (int c = 0; c < col; ++c) {
      int py = 0;
      for (int i = 0; i < prev_hss[c].size(); ++i) {
        int ny = py + prev_hss[c][i];
        fill(bottom_sep.begin() + py + 1, bottom_sep.begin() + ny + 1, ny);
        fill(top_sep.begin() + py, top_sep.begin() + ny, py);
        fill(sep_idx.begin() + py, sep_idx.begin() + ny, i);
        py = ny;
      }
      bottom_sep[0] = 0;
      top_sep[W] = W;
      sep_idx[W] = prev_hss[c].size();
      const int width = ws[c];
  ...
  vi place_separator(const vi& areas) {
    ...
    for (int i = 0; left > 0; ++i) {
      ret[areas.size() - 1 - i % areas.size()]++;
      left--;
    }
    for (int i = 0; left < 0; ++i) {
      if (ret[areas.size() - 1 - i % areas.size()] == 1) continue;
      ret[areas.size() - 1 - i % areas.size()]--;
      left++;
    }
    return ret;
